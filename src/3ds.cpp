...
//		MessageBox(NULL, strMessage, "Error", MB_OK);
	...
	for (int i=0; i<m_3DModel.numOfMaterials; i++)
	{
		...
		{
      m_3DModel.pMaterials[i].texureId =
          GLAid::loadTexture(QString(m_3DModel.pMaterials[i].mapName.string));
		}
	}
...
// 计算两向量的叉积
Vector3 C3DSModel::Cross(Vector3 v1, Vector3 v2)
{
	Vector3 vCross;
...
// 向量单位化
Vector3 C3DSModel::Normalize(Vector3 vNormal)
{
	double Magnitude;
...
//		{
//		case 要匹配的子ChunkID:
//			该子Chunk的读取函数或内容处理程序段;
//			break;
//		case 另一个要匹配的子ChunkID:
//			该子Chunk的读取函数或内容处理程序段;
//			break;
//		default:(跳过无用的Chunk)
//			fseek()使文件指针跳过该Chunk;
...
//2.增加结构体的内容
//3.增加对子Chunk的读取函数
//4.在这个子Chunk的父Chunk中增加case语句
//	例子：ReadPrimary()函数的注释部分
//5.完成
